...
int main(){
    ...
    ll count_a = sum_b - sum_a, count_b = sum_b - sum_a;
    rep(i, n){
        if(a[i] > b[i]) count_b -= a[i] - b[i];
        else if(a[i] < b[i]){
            if((b[i] - a[i]) % 2){
                count_a -= (b[i] - a[i]) / 2 + 1;
                count_b--;
            }
            else count_a -= (b[i] - a[i]) / 2;
        }
    }
    cout << ((count_a < 0 || count_b < 0) ? "No" : "Yes") << endl;
}
