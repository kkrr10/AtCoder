...
int main(){
    double n, x0, y0, x_, y_;
    cin >> n >> x0 >> y0 >> x_ >> y_;
    double x_center = (x0 + x_) / 2.0, y_center = (y0 + y_) / 2.0;
    cout << fixed << setprecision(7) << cos(2.0 * M_PI / n) * (x0 - x_center) - sin(2.0 * M_PI / n) * (y0 - y_center) + x_center << ' ' << sin(2.0 * M_PI / n) * (x0 - x_center) + cos(2.0 * M_PI / n) * (y0 - y_center) + y_center << endl;
}
