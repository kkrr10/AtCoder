...
int main(){
    ll n, key_index; cin >> n;
    rep(i, n){
        cin >> arr[i].x >> arr[i].y >> arr[i].h;
        if(arr[i].h) key_index = i;  // important
    }

    rep(cx, 101){
        rep(cy, 101){
            bool ok = true;
            ll h_tmp = arr[key_index].h + abs(arr[key_index].x - cx) + abs(arr[key_index].y - cy);
            rep(i, n){
                if(arr[i].h){
                    if(h_tmp - abs(arr[i].x - cx) - abs(arr[i].y - cy) != arr[i].h){
                        ok = false;
                        break;
                    }
                }
                else{
                    if(h_tmp - abs(arr[i].x - cx) - abs(arr[i].y - cy) > 0){
                        ok = false;
                        break;
                    }
                }
