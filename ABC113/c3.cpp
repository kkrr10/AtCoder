...
int main(){
    ll n, m, p, y; cin >> n >> m;
    vector<pll> input_vec, sorted_vec;
    map<ll, ll> count_in_city, year_to_order;
    rep(i, m){
        cin >> p >> y;
        input_vec.push_back(make_pair(y, p));
        sorted_vec.push_back(make_pair(y, p));
    }
    sort(all(sorted_vec));
    rep(i, m) year_to_order[sorted_vec[i].first] = ++count_in_city[sorted_vec[i].second];
    rep(i, m) printf("%06lld%06lld\n", input_vec[i].second, year_to_order[input_vec[i].first]);
}
