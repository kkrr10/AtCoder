...
int main(){
    ll n, a, ans = 0, min_abs = INF; cin >> n;
    bool found_zero = false, minus_even = true;
    rep(i, n){
        cin >> a;
        if(a == 0) found_zero = true;
        else if(a < 0) minus_even = !minus_even, a = -a;
        chmin(min_abs, a);
        ans += a;
    }
    if(found_zero || minus_even) cout << ans << endl;
